 ...
 * Waits for the avatar image to appear (ensures JavaScript has loaded).
 */
...
// --- DEBUG route: see raw HTML (temporary, remove later) ---
app.get('/debug/html/:username', async (req, res) => {
  try {
    const username = req.params.username;
    const url = `https://www.tikvib.com/profile/${username}`;
    const html = await fetchPageWithBrowserless(url);
    res.set('Content-Type', 'text/plain');
    res.send(html.substring(0, 6000));  // first 6000 chars, enough for profile section
  } catch (error) {
